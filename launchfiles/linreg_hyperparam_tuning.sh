...
expdir="/scratch/gobi1/adragnar/experiments/causal_discovery/0609_fulllinreg_test/$(date +'%s')"  #"/scratch/hdd001/home/adragnar/experiments/causal_discovery/$(date +'%s')"  #"/scratch/gobi1/adragnar/experiments/causal_discovery/$(date +'%s')"
mkdir -p $expdir
...
lambdas=(0.0000000001 0.00001 0.0001 0.001 0.1 1)
  ...
  do
    ...
    do
      ...
      do
        ...
        python setup_params.py $id $algo $data $expdir $cmdfile $paramfile -fteng $ft_combos -reduce_dsize $reduce_dsize -binarize $bin -inc_hyperparams 1 -seed $s -test_info $t -val_split $val_split -linreg_lambda $l
        id=$(($id + 1))
